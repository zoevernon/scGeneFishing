...
geneFishingDist <- function(exp_mat, bait_genes, pool_genes, 
  ...
  fish_vec <- foreach(sub_pool = sub_pool_list, .combine = 'c') %dopar% {
    result <- tryCatch({
      ...
      dist_mat_bait_and_pool <- stats::dist(exp_mat[c(bait_indices, 
                                               sub_indices), ] %>%
                                       as.matrix(), 
                                     method = method) %>%
        as.matrix()
      ...
      # above 
      rs <- getSpectralCoordinates( 1/ (dist_mat_bait_and_pool + 1), k)
